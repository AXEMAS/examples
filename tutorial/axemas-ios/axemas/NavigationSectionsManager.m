...
+ (void)goto:(NSDictionary*)data animated:(BOOL)animated {
    ...
    if([data objectForKey:@"stackMaintainedElements"] != nil){
        NSLog(@"stackMaintainedElements found");
...
+ (NSUInteger) getStackElements{
    return [[[NavigationSectionsManager activeNavigationController] viewControllers] count];
}
